# Chrome Proxy Manager

Современное расширение для Chrome с удобным управлением прокси-серверами, автоматическими обновлениями, DNS маршрутизацией и красивым интерфейсом.

[🇺🇸 English version](README.md)

## Возможности

### 🚀 Основная функциональность
- **Управление прокси-серверами**: Добавление, редактирование и удаление серверов
- **Поддержка протоколов**: HTTP/HTTPS
- **Переключение одним кликом**: Включение/выключение прокси одной кнопкой
- **Выбор сервера**: Быстрое переключение между настроенными серверами

### 🌐 Расширенные функции
- **DNS через прокси**: Опциональная маршрутизация DNS запросов через прокси (по умолчанию отключено для производительности)
- **Списки исключений**: Настройка доменов и IP, которые обходят прокси
- **Автообновления**: Автоматическая проверка обновлений расширения через GitHub API
- **Умная система повторов**: Механизм повторов с экспоненциальной задержкой при неудачных проверках
- **Миграция настроек**: Автоматическая миграция настроек при обновлении расширения

### 🎨 Пользовательский интерфейс
- **Современный дизайн**: Красивый интерфейс popup с градиентами
- **Отображение версии**: Текущая версия показана в заголовке
- **Статус в реальном времени**: Живые индикаторы состояния прокси
- **Адаптивная верстка**: Оптимизировано для разных размеров экрана

### 🔧 Технические особенности
- **Manifest V3**: Построено на последних стандартах Chrome расширений
- **Service Worker**: Эффективная фоновая обработка
- **Sync Storage**: Настройки синхронизируются между устройствами
- **Резервное копирование и восстановление**: Автоматическое создание резервных копий и восстановление
- **Обработка ошибок**: Комплексная обработка ошибок с понятными сообщениями

## Установка

### Из исходного кода

1. **Клонируйте репозиторий**:
   ```bash
   git clone https://github.com/kalkalch/chromeproxy.git
   cd chromeproxy
   ```

2. **Установите зависимости** (если нужно):
   ```bash
   make install-deps
   ```

3. **Соберите расширение**:
   ```bash
   make build
   ```

4. **Загрузите в Chrome**:
   - Откройте Chrome и перейдите в `chrome://extensions/`
   - Включите "Режим разработчика"
   - Нажмите "Загрузить распакованное расширение" и выберите папку `build/`

### Из релиза

1. Скачайте последний релиз с [GitHub Releases](https://github.com/kalkalch/chromeproxy/releases)
2. Распакуйте ZIP файл
3. Загрузите распакованную папку в Chrome как описано выше

## Использование

### Базовая настройка

1. **Нажмите на иконку расширения** в панели инструментов Chrome
2. **Добавьте прокси-сервер**:
   - Нажмите кнопку "+"
   - Введите данные сервера (название, тип, хост, порт)
   - Опционально добавьте список исключений (домены/IP для обхода прокси)
   - Нажмите "Сохранить"

3. **Включите прокси**:
   - Переключите главный выключатель для включения прокси
   - Расширение автоматически использует выбранный сервер

### Управление серверами

- **Редактировать сервер**: Нажмите иконку редактирования рядом с любым сервером
- **Удалить сервер**: Нажмите иконку удаления (корзина)
- **Переключить серверы**: Нажмите на любой сервер, чтобы сделать его активным

### Дополнительные опции

#### DNS через прокси
- **Расположение**: Раздел настроек в popup
- **По умолчанию**: Отключено (рекомендуется для лучшей производительности)
- **Когда включать**: Только если нужна маршрутизация DNS запросов через прокси
- **Предупреждение**: Может замедлить работу браузера

#### Списки исключений
Настройте домены и IP-адреса, которые должны обходить прокси:
```
example.com
*.google.com
192.168.1.0/24
localhost
```

### Автообновления

Расширение автоматически проверяет обновления каждый час. Вы можете:
- **Ручная проверка**: Нажать кнопку "Проверить обновления"
- **Переключить автопроверку**: Включить/выключить автоматическую проверку
- **Просмотр статуса**: Увидеть время последней проверки и текущую версию

## Разработка

### Система сборки

Проект использует Makefile для автоматизации сборки:

```bash
# Собрать расширение
make build

# Очистить артефакты сборки
make clean

# Сгенерировать иконки из SVG
make icons

# Создать пакет для распространения
make package

# Создать GitHub релиз
make release

# Установить Python зависимости
make install-deps

# Валидировать манифест
make validate

# Показать справку
make help
```

### Структура проекта

```
chromeproxy/
├── background.js          # Service worker (управление прокси)
├── popup.html             # Интерфейс popup расширения
├── popup.js               # Логика popup и обработка UI
├── popup.css              # Стили и анимации
├── manifest.json          # Манифест расширения
├── icons/                 # Файлы иконок
│   ├── icon.svg           # Исходная SVG иконка
│   ├── icon16.png         # Иконка 16x16
│   ├── icon32.png         # Иконка 32x32
│   ├── icon48.png         # Иконка 48x48
│   └── icon128.png        # Иконка 128x128
├── build/                 # Папка вывода сборки
├── .github/workflows/     # GitHub Actions CI/CD
├── Makefile              # Автоматизация сборки
├── create_icons.py       # Скрипт генерации иконок
└── README.md             # Английская документация
```

### GitHub Actions

Проект включает автоматизированный CI/CD:
- **Триггер сборки**: Push в ветку master (исключая изменения в docs/config)
- **Создание артефактов**: Сборки хранятся 30 дней
- **Авто-релиз**: Создает релизы для тегов версий (v*)
- **Ресурсы релиза**: Автоматически прикрепляет ZIP файлы к релизам

### Интеграция API

#### GitHub API
- **Endpoint**: `https://api.github.com/repos/kalkalch/chromeproxy/releases/latest`
- **Лимиты запросов**: 60 запросов в час для неаутентифицированных запросов
- **Заголовки**: Включает User-Agent для лучшего rate limiting
- **Обработка ошибок**: Умные повторы с экспоненциальной задержкой

## Конфигурация

### Типы прокси

| Тип | Описание | Случай использования |
|-----|----------|---------------------|
| HTTP/HTTPS | Объединенный HTTP и HTTPS прокси | Веб-браузинг, API вызовы |

### Хранение настроек

Настройки хранятся в Chrome sync storage и включают:
- **Прокси серверы**: Список настроенных серверов
- **Активный сервер**: ID текущего выбранного сервера
- **Состояние прокси**: Статус включен/выключен
- **DNS настройки**: Предпочтения DNS через прокси
- **Настройки обновлений**: Предпочтения автопроверки и тайминги
- **Отслеживание версий**: Для целей миграции

### Система миграции

Расширение включает автоматическую миграцию настроек:
- **Отслеживание версий**: Обнаруживает обновления расширения
- **Обратная совместимость**: Сохраняет существующие настройки
- **Слияние по умолчанию**: Добавляет новые настройки со значениями по умолчанию
- **Создание резервных копий**: Создает резервные копии перед миграциями

## Устранение неполадок

### Частые проблемы

#### Прокси не работает
1. Проверьте доступность прокси-сервера
2. Проверьте учетные данные сервера (если требуются)
3. Проверьте список исключений на конфликтующие записи
4. Попробуйте отключить DNS через прокси

#### Ошибки проверки обновлений
- **Ограничение запросов**: Подождите час, если видите ошибки лимита запросов
- **Проблемы с сетью**: Проверьте подключение к интернету
- **Недоступность GitHub**: Попробуйте позже

#### Потеря настроек после обновления
- Расширение должно автоматически мигрировать настройки
- Проверьте статус синхронизации Chrome
- Попробуйте отключить и снова включить расширение

### Отладочная информация

Включите Chrome DevTools для расширения:
1. Перейдите в `chrome://extensions/`
2. Найдите "Chrome Proxy Manager"
3. Нажмите "Inspect views: service worker"
4. Проверьте консоль на сообщения об ошибках

## Участие в разработке

Мы приветствуем участие! Вот как начать:

1. Сделайте fork репозитория
2. Создайте ветку функции: `git checkout -b feature-name`
3. Внесите изменения
4. Тщательно протестируйте
5. Отправьте pull request

### Стиль кода

- **Комментарии**: Всегда на английском
- **Переменные**: Используйте camelCase
- **Функции**: Описательные имена
- **Обработка ошибок**: Всегда включайте блоки try-catch
- **Логирование**: Используйте console.log для отладки

## История изменений

См. [CHANGELOG.md](CHANGELOG.md) для подробного списка изменений и истории версий.

## Поддержка

- **Проблемы**: [GitHub Issues](https://github.com/kalkalch/chromeproxy/issues)
- **Обсуждения**: [GitHub Discussions](https://github.com/kalkalch/chromeproxy/discussions)

## Лицензия

Этот проект является открытым исходным кодом и доступен под [MIT License](https://opensource.org/licenses/MIT).

## Благодарности

- Документация Chrome Extension APIs
- GitHub API для проверки обновлений
- Отзывы и вклад сообщества 